import {
  ...
  NativeHeliumConfig, PaywallEntitledEvent, PaywallEventHandlers, PaywallInfo, PaywallSkippedEvent, PaywallSkippedReason, PaywallUnavailableEvent, PresentUpsellParams,
  ResetHeliumOptions,
...
function addPaywallUnavailableEventListener(listener: (event: PaywallUnavailableEvent) => void): EventSubscription {
...
/** In-flight teardown. An initialize issued mid-reset must wait for it, or the reset's cleanup swallows the new initialization. */
...
function setupEventListeners(config: HeliumConfig) {
  ...
  addEntitledEventListener((event) => {
    try {
      ...
      const presentation = presentationFor(event?.presentationId, (candidate) => candidate.onEntitled !== undefined);
      const onEntitled = presentation?.onEntitled;
      if (presentation) {
        presentation.onEntitled = undefined;
      }
      if (onEntitled) {
        if (presentation && (isSkip || presentation.closed)) {
  ...
  addPaywallSkipEventListener((event) => {
    try {
      ...
      if (event.skipReason === 'alreadyEntitled' && presentation?.onEntitled) {
        return;
      }
      dispatchPaywallSkip(event, presentation);
    } catch (error) {
  ...
  addPaywallUnavailableEventListener((event) => {
    try {
      const presentation = presentationFor(event.presentationId, (candidate) => !candidate.opened && !candidate.closed);
      if (!presentation) {
        return;
      }
...
function latestPresentation(predicate: (presentation: Presentation) => boolean = () => true): Presentation | undefined {
...
export const presentUpsell = ({
  ...
  if (!pendingNativeInit && !pendingReset) {
    present();
    return;
  }
  ...
  // Wait out any in-flight reset first: an initialize issued mid-reset has not published its
  // handoff yet, and the present must queue behind that handoff once it exists.
...
function dispatchPaywallSkip(
  ...
  presentation: Presentation | undefined = presentationFor(event?.presentationId, (candidate) => candidate.onPaywallSkip !== undefined),
) {
  ...
  try {
    ...
    console.error('[Helium] onPaywallSkip callback failed', error);
  }
}
...
export const resetHelium = async (options?: ResetHeliumOptions): Promise<void> => {
  ...
  pendingReset = resetting;

  try {
    await resetting;
  } finally {
    // Overlapping resets each publish their own promise; only the latest owns the slot.
    if (pendingReset === resetting) {
 ...
 * whether it succeeded, was cancelled, or failed. If a user returns to the app manually without
 * the redirect URL, then the SDK will look at the latest entitlement state to determine if a
 * purchase was made.
 *
...
function enableExternalWebCheckout(options: {
  ...
  if (paymentProcessors && paymentProcessors.length === 0) {
    console.error("[Helium] enableExternalWebCheckout: paymentProcessors must not be empty. Pass ['paddle'], ['stripe'], or both.");
    return;
  }
  try {
    if (redirectURL !== undefined) {
      ...
      HeliumPaywallSdkModule.enableExternalWebCheckout(redirectURL, paymentProcessors);
      return;
    }
    if (!successURL || !cancelURL) {
      console.error('[Helium] enableExternalWebCheckout: successURL and cancelURL must not be empty.');
      return;
    }
    HeliumPaywallSdkModule.enableExternalWebCheckoutSuccessAndCancel(
      successURL,
      cancelURL,
      paymentProcessors,
    );

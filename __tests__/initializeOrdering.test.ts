...
describe('presentUpsell ordering against initialize', () => {
  ...
  it('holds a present issued while a reset and re-initialize are both in flight', async () => {
    const { helium, native, fileSystem } = loadHelium();
    const initializing = helium.initialize(CONFIG);
    fileSystem.__finishWrite();
    await initializing;
    ...
    native.resetHelium.mockImplementation(() => new Promise<void>((resolve) => {
      finishReset = resolve;
    }));
    void helium.resetHelium();
    void helium.initialize(CONFIG_AFTER_RESET);
    helium.presentUpsell({ triggerName: 'go_online' });
    await flushMicrotasks();
    ...
    expect(native.presentUpsell).toHaveBeenCalledTimes(1);
    expect(native.initialize.mock.invocationCallOrder[1])
      .toBeLessThan(native.presentUpsell.mock.invocationCallOrder[0]);
  });
});
...
describe('initialize against callers that await it', () => {
  ...
  it('holds an initialize issued during an in-flight reset until the reset completes', async () => {
    const { helium, native, fileSystem } = loadHelium();
    const initializing = helium.initialize(CONFIG);
    fileSystem.__finishWrite();
    await initializing;
    ...
    const resetting = helium.resetHelium();
    const reinitializing = helium.initialize(CONFIG_AFTER_RESET);
    await flushMicrotasks();
